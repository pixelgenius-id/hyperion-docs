...
***
!!!tip
    For more details and live testing, check out the [Vexanium Swagger](https://vexascan.com/v2/docs){:target="_blank"}
...
##### Examples
````
/v2/history/get_abi_snapshot?contract=vexcore&fetch=true
````
...
!!! note
    Besides the parameters listed above, this endpoint also accepts generic parameters based on indexed fields (e.g. `act.authorization.actor=vexcore` or `act.name=delegatebw`). 
    
    If included they will be combined with an AND operator.
 
!!!tip
    You can check action mappings on [index-templates.ts](https://github.com/pixelgenius-id/hyperion-history-api/blob/main/definitions/index-templates.ts){:target="_blank"} file for the list of all possible parameters.
...
##### Examples
````
/v2/history/get_actions?account=vexcore&act.name=delegatebw
````
...
##### Examples
```
/v2/history/get_created_accounts?account=vexcore
```
...
##### Examples
```
/v2/history/get_creator?account=vexaniumcore
```
...
!!!tip
    You can check delta mappings on [index-templates.ts](https://github.com/pixelgenius-id/hyperion-history-api/blob/main/definitions/index-templates.ts){:target="_blank"} file for the list of all possible parameters.
...
##### Examples
Get all deltas from `vexcore` contract on a specific block
```
/v2/history/get_deltas?code=vexcore&block_num=409600000
```

Get all deltas from the table `accounts` of the `vexcore` contract 
```
/v2/history/get_deltas?code=vexcore&table=accounts
```
...
##### Examples
```
/v2/history/get_transacted_accounts?account=vexaniumcore&direction=out
```
...
Get token transfers from contracts using the token standard.
...
##### Examples
Get all transfers from `vexaniumcore` account
```
/v2/history/get_transfers?from=vexaniumcore
```

Get all transfers from `45g2lkptw412` account to `swap.wind` account
```
/v2/history/get_transfers?from=45g2lkptw412&to=swap.wind"
```

Get all transfers from `45g2lkptw412` account to `swap.wind` account after November 2019
```
/v2/history/get_transfers?from=45g2lkptw412&to=swap.wind&after=2026-01-01T00:00:00.000Z
```
...
##### Examples
```
/v2/state/get_account?account=vexcore
```
...
##### Examples
```
/v2/state/get_links?account=vexaniumcore
```
...
##### Examples
```
/v2/state/get_proposals?proposer=vexaniumcore
```
...
##### Examples
```
/v2/state/get_tokens?account=vexaniumcore
```
...
##### Examples
Get all `vexaniumcore` voters
```
/v2/state/get_voters?producer=vexaniumcore
```

Get only the first 3 responses
```
/v2/state/get_voters?producer=vexaniumcore&limit=3
```
